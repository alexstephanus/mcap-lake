#! /usr/bin/env python3
""" Script to download public logs, taken from PX4 and licensed
under BSD-3, rather than the MIT license that governs rest of the project.
See data/LICENSE.md for the original script's license terms.

The following modifications have been put in place:
1. File names are saved locally as <flight_date>-<log_id>.ulg, rather than <log_id>.ulg.
   There are corresponding changes to the cache-matching logic to look for date-and-uuid
   matches, rather than blindly matching filename.
2. The --skip-if-cached has been added, to prevent downloading more files when you already
   have some in local storage (they pay for the servers, so being judicious with downloads
   is good behavior).
3. The threshold to trigger a warning is now 50 files, rather than 100, for the same reason as above
4. the --flight-modes and --error-labels arguments have been removed due to them relying on
   other pieces of the original application and not being necessary.
5. The default download folder has been changed from `data/downloaded` to `data/raw`
"""
...
WARN_THRESHOLD = 50       # Warn user if downloading more than this many files
...
def get_arguments():
    ...
    parser.add_argument('--skip-if-cached', type=int, default=0,
                        help='Idempotency short-circuit, independent of --max-num: if the '
                             'download folder already holds at least this many .ulg files, skip '
                             'the run entirely without hitting the database. 0 (default) disables '
                             'it.')
    parser.add_argument('-d', '--download-folder', type=str, default="data/raw/",
                        help='The folder to store the downloaded logfiles. This is also the '
                             'local cache that seeds the MinIO ulog bucket (see docker-compose '
                             'minio-init), so MinIO can be wiped and re-seeded without '
                             're-downloading from the PX4 server.')
...
def log_id_from_filename(stem):
    """
    Extract the log_id from a cached logfile's stem. Filenames are
    {log_date}-{log_id}; log_date is a fixed YYYY-MM-DD prefix, so strip it to
    recover the log_id used for de-dup against the durable data/raw cache.
    """
    match = re.match(r'\d{4}-\d{2}-\d{2}-(.+)', stem)
    return match.group(1) if match else stem


def count_cached_logs(download_folder):
    """Number of .ulg files already in the cache folder."""
    return len(glob.glob(os.path.join(os.path.abspath(download_folder), "*.ulg")))
...
def main():
    ...
    # Idempotency short-circuit (opt-in via --skip-if-cached): the cache is durable, so once
    # it holds enough logs there's nothing to do — skip the DB hit entirely. Independent of
    # --max-num. --overwrite (force re-download) and --print (inspect the DB) bypass it.
    if args.skip_if_cached > 0 and not args.overwrite and not args.print_entries:
        cached = count_cached_logs(args.download_folder)
        if cached >= args.skip_if_cached:
            print(f"Already have {cached} logs in {args.download_folder} "
                  f"(>= --skip-if-cached {args.skip_if_cached}); skipping download.")
            return
    ...
    if args.print_entries:
        ...
        logids = frozenset(
            log_id_from_filename(os.path.splitext(os.path.basename(f))[0]) for f in logfiles)
        ...
        for i in range(n_en):
            ...
            log_date = db_entries_list[i]['log_date']
            file_path = os.path.join(args.download_folder, f"{log_date}-{entry_id}.ulg")
